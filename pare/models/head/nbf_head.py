...
from ..layers.non_local import concatenation, dot_product, embedded_gaussian, gaussian
from ..backbone.resnet import conv3x3, conv1x1, BasicBlock

BN_MOMENTUM = 0.1


class NBFHead(nn.Module):
    def __init__(
            self,
            num_joints,
            num_input_features,
        ...
        shape_mlp_inp_dim = num_deconv_filters[-1] # num_joints * smpl_final_dim
        ...
        self.pose_mlp = self._get_shape_mlp(output_size=24*6)
        # self._get_pose_mlp(num_joints=num_joints, output_size=6)

        self.part_downsample = self._make_downsample_layer(
            num_layers=3,
            num_channel=num_deconv_filters[-1],
            num_joints=num_joints
        )

    def _make_downsample_layer(self, num_layers, num_channel, num_joints, kernel_size=3):
        layers = []
        for i in range(num_layers):
            if i == 0:
                layers.append(
                    nn.Conv2d(
                        in_channels=num_joints, out_channels=num_channel,
    ...
    def forward(self, features):
        ...
        point_local_feat = cam_shape_feats = self.part_downsample(part_attention.detach()).reshape(-1, 256)
