...
from ...utils.geometry import get_coord_maps
...
class PareNPHead(nn.Module):
    def __init__(
        ...
        super(PareNPHead, self).__init__()
        ...
        if self.use_soft_attention:
            ...
            self.keypoint_final_layer = nn.Sequential(
                ...
                conv1x1(256, num_joints+1 if self.use_heatmaps == 'part_segm' else num_joints),
            )
            ...
            self.keypoint_final_layer = nn.Conv2d(
                ...
                out_channels=num_joints+1 if self.use_heatmaps == 'part_segm' else num_joints,
        ...
        #         'all_pose': 24 * 3, # rot6d of all joints from previous iter
        #         'self_pose': 3, # rot6d of self
        #         'neighbor_pose': 2 * 3, # rot6d of neighbor joints from previous iter
        ...
        self.pose_mlp = self._get_pose_mlp(num_joints=num_joints, output_size=3)
    ...
    def forward(self, features):
        ...
        point_local_feat = self._get_local_feats(smpl_feats, part_attention, output)

        ############## GET FINAL PREDICTIONS ##############
        pred_pose = self._get_final_preds(point_local_feat)
    ...
    def _get_final_preds(self, pose_feats):
        return self._pare_get_final_preds(pose_feats)

    def _pare_get_final_preds(self, pose_feats):
        ...
        pred_pose = pred_pose.squeeze(-1).transpose(2, 1) # N, J, 3
        return pred_pose